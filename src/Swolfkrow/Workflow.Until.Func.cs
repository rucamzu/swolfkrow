...
public partial class Workflow<TEvent>
{
   ...
   public Workflow<TEvent> Until(Func<TEvent, bool> predicate)
        => WorkflowImpl.UntilFromPredicate(_workflow, predicate).ToWorkflow();

    /// <summary>
    /// Interupts the asynchronous <see cref="Workflow{TEvent}"/> right after the first event that does not satisfy a predicate.
    /// </summary>
    ...
    /// <returns>A new asynchronous <see cref="Workflow{TEvent}"/> that yields the events yielded by <see langword="this"/> asynchronous workflow up to, and including, the first event that does not satisfy the given <paramref name="predicate"/>.</returns>
    public Workflow<TEvent> Until<TArg>(
        Func<TEvent, TArg, bool> predicate, TArg arg)
        => WorkflowImpl.UntilFromPredicate(_workflow, predicate.BindLast(arg)).ToWorkflow();
    ...
    public Workflow<TEvent> Until<TArg1, TArg2>(
        ...
        => WorkflowImpl.UntilFromPredicate(_workflow, predicate.BindLast(arg1, arg2)).ToWorkflow();
    ...
    public Workflow<TEvent> Until<TArg1, TArg2, TArg3>(
        ...
        => WorkflowImpl.UntilFromPredicate(_workflow, predicate.BindLast(arg1, arg2, arg3)).ToWorkflow();
}