...
public partial class Trigger<TEvent, TTriggerEvent1, TTriggerEvent2>
{
    ...
    /// </summary>
    /// <param name="effect">A function encapsulating an asynchronous side effect that takes two events of types <typeparamref name="TTriggerEvent1"/> and <typeparamref name="TTriggerEvent2"/>.</param>
    ...
    public Workflow<TEvent> Do(
        ...
        => WorkflowImpl.DoFromTriggered2ValueTask(
            _workflow, _predicate, effect).ToWorkflow();

    /// <summary>
    /// Injects an asynchronous side effect executed on every pair of yielded triggering events.
    /// </summary>
    /// <param name="effect">A function encapsulating an asynchronous side effect that takes two events of types <typeparamref name="TTriggerEvent1"/> and <typeparamref name="TTriggerEvent2"/> and one additional argument.</param>
    ...
    public Workflow<TEvent> Do<TArg>(
        ...
        => WorkflowImpl.DoFromTriggered2ValueTask(_workflow, _predicate, effect.BindLast(arg)).ToWorkflow();

    /// <summary>
    /// Injects an asynchronous side effect executed on every pair of yielded triggering events.
    /// </summary>
    /// <param name="effect">A function encapsulating an asynchronous side effect that takes two events of types <typeparamref name="TTriggerEvent1"/> and <typeparamref name="TTriggerEvent2"/> and two additional arguments.</param>
    ...
    public Workflow<TEvent> Do<TArg1, TArg2>(
        ...
        => WorkflowImpl.DoFromTriggered2ValueTask(_workflow, _predicate, effect.BindLast(arg1, arg2)).ToWorkflow();

    /// <summary>
    /// Injects an asynchronous side effect executed on every pair of yielded triggering events.
    /// </summary>
    /// <param name="effect">A function encapsulating an asynchronous side effect that takes two events of types <typeparamref name="TTriggerEvent1"/> and <typeparamref name="TTriggerEvent2"/> and three additional arguments.</param>
    /// <param name="arg1">The first argument passed to the given synchronous side <paramref name="effect"/>.</param>
    /// <param name="arg2">The second argument passed to the given synchronous side <paramref name="effect"/>.</param>
    ...
    public Workflow<TEvent> Do<TArg1, TArg2, TArg3>(
        ...
        => WorkflowImpl.DoFromTriggered2ValueTask(_workflow, _predicate, effect.BindLast(arg1, arg2, arg3)).ToWorkflow();
}
