...
{
    /// <summary>
    /// Continues an asynchronous workflow with an event. 
    /// </summary>
    /// <param name="workflow">An asynchronous workflow.</param>
    /// <param name="continuation">An <typeparamref name="TEvent"/> that will be yielded as continuation.</param>
    /// <typeparam name="TEvent">The (base) type of the events yielded by the asynchronous <paramref name="workflow"/>.</typeparam>
    /// <returns>An asynchronous workflow that yields the events yielded by the given asynchronous <paramref name="workflow"/>, followed by the given <paramref name="continuation"/> event.</returns>
    public static IAsyncEnumerable<TEvent> Then<TEvent>(this IAsyncEnumerable<TEvent> workflow,
        TEvent continuation)
        => workflow.Then(Workflow.Start(continuation));

    /// <summary>
    /// Continues an existing asynchronous workflow with an event factory. 
    /// </summary>
    ...
    /// <typeparam name="TEvent">The (base) type of the events yielded by the asynchronous <paramref name="workflow"/>.</typeparam>
    /// <returns>An asynchronous workflow that yields the events yielded by the given asynchronous <paramref name="workflow"/>, followed by the event returned by the given <paramref name="createContinuation"/> factory.</returns>
    public static IAsyncEnumerable<TEvent> Then<TEvent>(this IAsyncEnumerable<TEvent> workflow,
        Func<TEvent> createContinuation)
        => workflow.Then(Workflow.Start(createContinuation));

    /// <summary>
    /// Continues an existing asynchronous workflow with an event factory that takes one argument.
    /// </summary>
    /// <param name="workflow">An existing asynchronous workflow.</param>
    ...
    /// <summary>
    /// Continues an existing asynchronous workflow with an event factory that takes two arguments.
    /// </summary>
    /// <param name="workflow">An existing asynchronous workflow.</param>
    ...
    /// <summary>
    /// Continues an existing asynchronous workflow with an event factory that takes three arguments.
    /// </summary>
    /// <param name="workflow">An existing asynchronous workflow.</param>
